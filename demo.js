...
let articleHearts = document.querySelectorAll(".like");  // step 1 locate the page element
  ...
  mimicServerCall()
    .then(function(serverMessage){
      ...
      alert("You notified the server!");
       alert(serverMessage);
      heart.innerText = glyphStates[heart.innerText];  // cause a change to give code so that sever-side behavior is stubbed in(set up mock server communication)
...
for (let glyph of articleHearts) {       // cause a change to given code so that event listening has an effect
